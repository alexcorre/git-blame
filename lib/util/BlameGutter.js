...
export default class BlameGutter {
  ...
  constructor(editor) {
    ...
    // resize
    this.currentWidth = atom.config.get('git-blame.columnWidth');
  ...
  setVisibility(visible) {
    ...
    if (visible) {
      ...
      this.updateGutterWidth(this.currentWidth);
    } else {
  ...
  onResizeMove(e) {
    ...
    this.updateGutterWidth(this.currentWidth + delta);
  }
  ...
  bindResizeEvents() {
    ...
    if (!this.eventListeners['mousemove']) {
      const mouseMoveHandler = this.onResizeMove.bind(this);
  ...
  updateGutterWidth(newWidth) {
    console.log("** updating the gutter width to", newWidth, this.gutter());
    atom.config.set('git-blame.columnWidth', newWidth);

    const gutterElements = document.querySelectorAll('.gutter');
    const gutterEl = find(gutterElements, el => {
      return el.getAttribute('gutter-name') === GUTTER_ID;
    });
    if (gutterEl) {
      gutterEl.style.width = `${newWidth}px`;
    }
  }
