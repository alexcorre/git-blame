...
export default class RemoteRevision {
  ...
  getTemplate() {
    if (this.isGitHub()) {
      return this.safeTemplate(GITHUB_TEMPLATE);
    }

    if (this.isBitbucket()) {
      return this.safeTemplate(BITBUCKET_TEMPLATE);
    }

    if (this.isGitLab()) {
      return this.safeTemplate(GITLAB_TEMPLATE);
    }
