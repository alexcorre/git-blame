...
Blamer.prototype.initialize = function() {
  this.tools = {};
  this.tools.root = new GitTools(this.repo.getPath());

  var submodules = this.repo.submodules;
  if (submodules) {
    for (var submodulePath in submodules) {
      this.tools[submodulePath] = new GitTools(this.repo.getWorkingDirectory() + '/' + submodulePath);
    }
  }
}
...
Blamer.prototype.blame = function(filePath, callback) {
  ...
  // relativize the path again, just in case we now are using a submodule repo
  console.log('filePath: ' + filePath);
  console.log('repoPath: ' + repoUtil.path);
  ...
  repoUtil.blame(blameOptions, function(err, blame) {
    callback(err, blame);
  });
}

/**
 * Utility to get the GitTools repository for the given filePath. Takes into
...
Blamer.prototype.repoUtilForPath = function(filePath) {
  ...
  // By default, we return the root GitTools repository.
  var repoUtil = this.tools.root;

  // if we have submodules, loop through them and see if the given file path
  // belongs inside one of the repositories. If so, we return the GitTools repo
  // for that submodule.
  if (submodules) {
    for (var submodulePath in submodules) {
